...
change introduces. "Stock-bearing" means a state whose units are on hand and reach the ledger —
`GOOD` and `DAMAGED`. `UNUSABLE` has never been on hand and stays off the ledger.
