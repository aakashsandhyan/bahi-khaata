...
*Internal barcodes are `BBZ-` + a six-digit counter, from `BBZ-100000`.* Stock without a usable manufacturer code gets one generated. The `BBZ-` letter prefix is the entire collision guarantee: a manufacturer barcode is an all-numeric EAN-13/UPC, and a value containing letters can never equal one — no number range has to be reserved or tracked. Starting the counter at 100000 keeps every value six digits without zero-padding; exhaustion at 999999 fails loudly rather than emitting a seventh digit that would break label width. The counter is a single atomic-increment row (`internal_barcode_counter`), the one monotonic integer in a UUID-keyed schema, because internal codes must be dense and fixed-width. `origin` records which kind a barcode is, so lookups never parse the string. Rendering the value to a Code 128 label image (ZXing) belongs to labelling, out of scope here.
...
*Revised during implementation, at the user's direction:* a real seam was built rather than a single implementation behind a named interface. The original reasoning — that an abstraction with one implementation is read on every visit and exercised never — was sound in general but wrong here, because the seam turned out to be far narrower than expected.

Everything that is hard is *common* to every strategy: freight, removing pinned lines from the pool, exact reconciliation in paise, dividing a share into a unit cost, the guards. What varies is one question — how much is this line worth relative to its neighbours? So the interface is `AllocationWeighting`, a single method returning a `long` weight, and `CostAllocator` owns the rest. Adding a strategy costs one small class and cannot break reconciliation, because reconciliation is not its business. That is a seam worth having on day one; a full swappable-allocator abstraction would not have been.
...
*Recorded during implementation:* the line's **whole share** is stored alongside the unit cost (`allocated_total_paise`). A unit cost alone cannot satisfy "allocated costs sum to the lot amount exactly" — a ₹1.00 share over three units is ₹0.33 each, and 0.33 × 3 is ₹0.99, so a paise disappears on every line of every lot and the books drift for no discoverable reason. The share is authoritative and reconciles; the unit cost is derived from it by floor division and is what cost of goods sold uses. Floor rather than round, so multiplying the unit cost back out can only ever understate the line — never let COGS exceed what was actually paid.
...
- **Quantity on hand is a SUM over an ever-growing ledger** → Fine at this scale, and the `(product_id, effective_at)` index covers it. The ledger is append-only and never pruned: roughly half a million rows a year at a couple of hundred baskets a day. If the sum ever becomes slow, the standard remedy is periodic snapshot rows so a query sums forward from the last snapshot rather than from the beginning. Deliberately not built — it is speculative complexity today — but recorded so the escape hatch is known rather than rediscovered under pressure.
